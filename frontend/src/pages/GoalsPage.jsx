  ...
  return (
    <MainLayout>
      <div className="max-w-3xl mx-auto">
        ...
        {/* Header Section */}
        <div className="bg-[#113C3A] rounded-3xl p-8 text-white mb-8 shadow-lg relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#22B2B0]/20 rounded-full blur-[80px]"></div>
          <div className="relative z-10">
            <h1 className="text-3xl font-bold mb-2">Target Pemulihan</h1>
            <p className="text-[#A7F3D0]/80 text-sm">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-8">
          <form onSubmit={handleAddGoal} className="flex gap-3">
            <input 
              ...
              placeholder="Contoh: Jalan pagi 15 menit, Minum air putih..."
              className="flex-1 bg-[#F9FAFA] border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#22B2B0] focus:ring-1 focus:ring-[#22B2B0]"
              disabled={isLoading}
            />
            <button 
              type="submit" 
              disabled={isLoading || !newGoal.trim()}
              className="bg-[#22B2B0] hover:bg-[#1E9E9D] text-white px-6 py-3 rounded-xl font-semibold text-sm transition-colors disabled:opacity-50"
            >
        ...
        <div className="space-y-3">
          {goals.length === 0 ? (
            <div className="text-center py-10 bg-white rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-500 text-sm">Belum ada target. Yuk, buat langkah pertamamu hari ini!</p>
            </div>
          ) : (
            goals.map((goal) => (
              <div 
              ...
              >
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <span className={`text-sm font-medium ${goal.status === 'completed' ? 'text-gray-400 line-through' : 'text-gray-700'}`}>
                    {goal.title}
                  </span>
                </div>
                  ...
                  {goal.status === 'completed' ? (
                    ...
                    <>
                      <button 
                        ...
                        className="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-blue-50 hover:text-blue-500 transition-colors"
                        title="Edit Target"
                      >
